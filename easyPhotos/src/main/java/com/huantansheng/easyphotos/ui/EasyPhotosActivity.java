...
public class EasyPhotosActivity extends AppCompatActivity implements AlbumItemsAdapter.OnClickListener, PhotosAdapter.OnClickListener, AdListener, View.OnClickListener {
    ...
    private void initSomeViews() {
        ...
        findViewById(R.id.iv_second_menu).setVisibility(Setting.showPuzzleMenu || Setting.showCleanMenu || Setting.showOriginalMenu || Setting.showEditMenu ? View.VISIBLE : View.GONE);
...
//        super.onActivityResult(requestCode, resultCode, data);
        ...
        switch (resultCode) {
            case RESULT_OK:
                ...
                if (Code.REQUEST_EDIT == requestCode) {// 编辑完成
                    Photo photo = new Photo(null, null, mEditSavePath, 0L, 0, 0, 0, 0, null);
                    Result.photos.clear();
            ...
            case RESULT_CANCELED:
                ...
                if (Code.REQUEST_EDIT == requestCode && Setting.onlyStartCamera) {
                    finish();
                    return;
                }
                break;
    ...
    private void onCameraResultForQ() {
        ...
        if (Setting.onlyStartCamera || albumModel.getAlbumItems().isEmpty()) {
            if (!Setting.selectedOriginal) {//todo 非原图进入编辑页
                toEdit(photoUri);
//                finish();
                return;
            }
...
//                finish();
    ...
    private void initView() {
        ...
        setClick(tvAlbumItems, rootViewAlbumItems, tvDone, tvOriginal, tvPreview, ivCamera, tvEdit);
    ...
    public void onClick(View v) {
        ...
        if (R.id.tv_album_items == id || R.id.iv_album_items == id) {
            ...
            toEdit(UriUtils.getUri(this, new File(Result.getPhotoPath(0))));
        }
    }

    private void toEdit(Uri uri) {
        mEditSavePath = FileUtils.getFileCachePath(this, System.currentTimeMillis() + ".jpg");
        Intent intent = new Intent(this, IMGEditActivity.class);
        intent.putExtra(IMGEditActivity.EXTRA_IMAGE_URI, uri);
        intent.putExtra(IMGEditActivity.EXTRA_IMAGE_SAVE_PATH, mEditSavePath);
        if (Setting.autoToCropPic) {
            intent.putExtra(IMGEditActivity.MODIFY_MODE, IMGMode.CLIP);
        }
    ...
    private void shouldShowMenuDone() {
        ...
        tvEdit.setEnabled(Result.count() == 1 && Result.isImage(0));
        if (tvEdit.isEnabled()) {
            tvEdit.setTextColor(ContextCompat.getColor(this, R.color.easy_photos_fg_primary));
        } else {
            tvEdit.setTextColor(ContextCompat.getColor(this, R.color.easy_photos_fg_primary_dark));
        }
